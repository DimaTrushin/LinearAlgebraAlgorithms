...
Пусть дана линейная оболочка $V = \langle v_1, v_2, v_3\rangle \subseteq \mathbb R^4$, где
\[
v_{{1}} = \begin{pmatrix}
{2}\\
{5}\\
...
\begin {array}{cccc}
2&5&-8&-8\\
\noalign{\medskip}3&-9&-1&-1\\
\noalign{\medskip}-1&-4&5&5
\end {array}
\right)
\mapsto
\left(
...
Пусть дано подпространство $V = \langle v_1, v_2, v_3, v_4\rangle\subseteq \mathbb R^5$, где
\[
v_{{1}} = \begin{pmatrix}
{1}\\
{1}\\
...
\end{pmatrix}
\]
Количество ненулевых строк в ступенчатом виде меньше, чем количество исходных векторов.
Значит исходные векторы линейно зависимы и потому дополнить до базиса их нельзя.

Теперь возьмем подпространство $V = \langle v_1, v_2, v_3\rangle\subseteq \mathbb R^5$ для тех же самых векторов.
Положим их по строкам и приведем к ступенчатому виду:
\[
\begin{pmatrix}
1&1&6&-1&2\\
3&-2&3&3&3\\
...
\end{pmatrix}
\]
Количество ненулевых строк в ступенчатом виде не изменилось.
Значит $v_1, v_2, v_3$ линейно независимы и их можно дополнить до базиса $\mathbb R^5$.
Свободные позиции имеют номера $3$ и $5$.
Значит можно дополнить векторами $e_3$ и $e_5$ из стандартного базиса, то есть векторами
\[
e_3 =
\begin{pmatrix}
0\\0\\1\\0\\0
...
Пусть подпространство задано системой $V = \{y\in \mathbb R^5 \mid Ay = 0\}$, где
\[
A =
\begin{pmatrix}
1&2&4&-2&-3\\
...
\end{pmatrix}
\]
Свободные переменные в позициях $3$ и $5$.
Значит будет два вектора в ФСР.
Сначала заполним свободные позиции так, чтобы они давали стандартный базис
\[
\begin{aligned}
u_1 &= 
\begin{pmatrix}
...
Пусть задана линейная оболочка $V = \langle v_1, v_2, v_3, v_4\rangle \subseteq \mathbb R^5$, где
\[
v_{{1}} = \begin{pmatrix}
{-2}\\
{1}\\
...
\end{pmatrix}
\]
Положим векторы по строкам и образуем матрицу $B$.
Теперь найдем ФСР по предыдущему алгоритму.
\[
\begin{pmatrix}
-2&1&8&2&9\\
1&3&3&1&3\\
...
Пусть задано пространство $\mathbb R^3$ и два базиса:
\[
e_{{1}} = \begin{pmatrix}
{1}\\
{-1}\\
...
{2}\\
{-2}\\
\end{pmatrix},\;f_{{3}} = \begin{pmatrix}
{1}\\
{2}\\
{-3}\\
\end{pmatrix}
...
Пусть отображение $\varphi \colon \mathbb R^3\to \mathbb R^2$ задано по правилу $\varphi(x) = Ax$, где
\[
A =
\begin{pmatrix}
1&-1&-2\\
...
\end{pmatrix}
\]
В этом случае $A$ -- это матрица линейного отбражения в стандартных базисах пространств. Пусть $e_1, e_2, e_3$ и $f_1, f_2$ -- стандартные базисы в $\mathbb R^3$ и $\mathbb R^2$ соответственно. И пусть
\[
e_{{1}}' = \begin{pmatrix}
{1}\\
{-1}\\
...
Пусть в $\mathbb R^4$ даны следующие векторы
\[
"v_{{1}} = \begin{pmatrix}
{2}\\
{-1}\\
...
Пусть линейное отображение $\varphi \colon \mathbb R^4\to \mathbb R^3$ задано по правилу $\varphi(x) = Ax$, где
\[
A =
\begin{pmatrix}
-1&3&7&-4\\
...
Пусть в пространстве $\mathbb R^3$ задано подпространство $W = \langle b_1, b_2\rangle$, где
\[
b_1 = \begin{pmatrix}
{2}\\
{3}\\
...
\end{pmatrix}
\]
Наша задача найти $\varphi \colon \mathbb R^4\to \mathbb R^3$ такое, что $\Im \varphi = W$ и $\ker \varphi = U$. Для этого составим матрицу $B = (b_1|b_2)$ и тогда искомое линейное отображение $\varphi \colon \mathbb R^4\to \mathbb R^3$ будет задано $\varphi(x) = Dx$, где
\[
D = BA =
\begin{pmatrix}
2&1\\
...
Пусть в $\mathbb R^5$ заданы два подпространства $V = \langle v_1, v_2, v_3\rangle$ и $U = \langle u_1, u_2, u_3\rangle$, где
\[
v_{{1}} = \begin{pmatrix}
{1}\\
{3}\\
...
\end{pmatrix}
\]
Кроме того, так как $u_1, u_2, u_3$ (это можно проверить) линейно независимы и $\beta_1, \beta_2$ линейно независимы (это очевидно по построению), то полученные векторы $w_1, w_2$ будут линейно независимы. Однако, в общем случае эти векторы НЕ обязаны быть базисом $V\cap U$.
...
\subsection{Проверка на диагонализуемость над $\mathbb C$}

\paragraph{Дано}

Матрица $A\in \Matrix{\mathbb C}{n}$, задающая линейный оператор $\varphi \colon \mathbb C^n\to\mathbb C^n$.%
\footnote{На самом деле в этом алгоритме важно, чтобы базовое поле $F$ было алгебраически замкнутым и характеристики $0$.}

\paragraph{Задача}

Выяснить существует ли базис, в котором $\varphi$ задается диагональной матрицей и если задается, то какой именно.
На матричном языке: существует ли невырожденная матрица $C\in \Matrix{\mathbb C}{n}$ такая, что $C^{-1}AC$ является диагональной и найти эту диагональную матрицу.

\paragraph{Алгоритм}

\begin{enumerate}
\item Найдем характеристический многочлен $\chi(t)$ для $\varphi$, он же для $A$ по формуле $\chi(t) = (-1)^n\det(A-t E)$.

\item Избавимся от кратных корней в хар многочлене по правилу $h = \frac{\chi}{(\chi', \chi)}$, где в 
знаменателе стоит НОД хар многочлена и производной.%
\footnote{Вот в этом месте нам надо, чтобы поле было характеристики $0$. В противном случае мы могли полностью исключить из $h$ множители вида $(t-\lambda)^{pn}$, где $p$ -- характеристика поля.}

\item Проверим зануляет ли $h$ оператор, то есть выполнено ли $h(A) = 0$. Если выполнено, то оператор диагонализуем,%
\footnote{Если поле НЕ алгебраически замкнутое, то еще надо проверить, что $\chi_A$ раскладывается на линейные множители.}
если нет, то не диагонализуем.
\end{enumerate}

Главное преимущество этого алгоритма в том, что в нем не надо искать корни многочлена.
